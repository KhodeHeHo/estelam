...
const rules={
    zarori:(v)=> !!v ||'ضروری است',
    adad:(v)=> !isNaN(v) ||  'عدد وارد کنید',
    fiveChr:(v)=> v.length<=5 || 'حداکثر 5 کاراکتر',
    onlyPersian:(v)=>{
      for(let i=0;i<v.length;i++){
        if (v.charCodeAt(i)>1740 || v.charCodeAt(i)<1570) return 'فقط فارسی تایپ کنید!'
      }
      return true
    }
  }
